...
export class GetRuleEndpoint extends OpenAPIRoute {
	schema = {
		tags: ["Rules"],
		summary: "Get a rule by name or organization/name",
		security: [{ bearerAuth: [] }],
		request: {
			params: z.object({
				rulename: z.string(),
	...
	async handle(c: AppContext) {
		...
		const { rulename } = data.params;
		const { org, version } = data.query;
