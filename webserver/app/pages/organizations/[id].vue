...
type User = UserType;
type Member = User & { role: "owner" | "member"; joinedAt?: number };
...
const fetchOrganizationDetails = async () => {
	...
	try {
		...
		if (activeTab.value === "rules") {
			...
			rules.value = rulesData as any; // API returns different shape
		}

		// Fetch members if on members tab
		if (activeTab.value === "members") {
			const membersData = await $rpc.organizations.members({ organizationId });
			members.value = membersData as any; // API returns different shape
		}
	} catch (error) {
