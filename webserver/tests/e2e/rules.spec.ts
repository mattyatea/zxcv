...
import { waitForToast, generateUniqueId, waitForNetworkIdle } from "./helpers/test-utils";
...
test.describe("Rule Management", () => {
	...
	test.describe("Rule Creation", () => {
		test("should create a new rule successfully", async ({ page }) => {
			...
			await page.fill('[data-testid="rule-content"]', "# Test Rule\n\nThis is the rule content in markdown.");
			...
			await expect(page.locator('[data-testid="rule-description"]')).toContainText("This is a test rule description");
		});
		...
		test("should validate required fields", async ({ page }) => {
			...
			await expect(page.locator('[data-testid="name-error"]')).toContainText("Rule name is required");
			await expect(page.locator('[data-testid="content-error"]')).toContainText("Rule content is required");
		});
	...
	test.describe("Rule Editing", () => {
		test("should edit an existing rule", async ({ page }) => {
			...
			await expect(page.locator('[data-testid="rule-description"]')).toContainText(updatedDescription);
		});
		...
		test("should show version history", async ({ page }) => {
			...
			await expect(page.locator('[data-testid="version-item"]')).toHaveCount(await page.locator('[data-testid="version-item"]').count());
		});
	});
	...
	test.describe("Rule Deletion", () => {
		test("should delete a rule with confirmation", async ({ page }) => {
			...
			await expect(page.locator('[data-testid="confirm-message"]')).toContainText("Are you sure you want to delete this rule?");
